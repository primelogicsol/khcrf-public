...
import { publicationsHeroFallback } from "@/config/heroFallbacks";
...
export const metadata: Metadata = {
  title: "Publications & Research Catalog | KHCRF Knowledge Hub",
...
export default async function PublicationsPage(props: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  if (categoryParam) {
    ...
    const dest = categoryMapping[categoryParam];
    if (dest) {
      const newParams = new URLSearchParams();
      for (const key in searchParams) {
        if (key !== 'category') {
          const val = searchParams[key];
          if (Array.isArray(val)) val.forEach(v => newParams.append(key, v));
          else if (val) newParams.set(key, val);
        }
      }
      const qs = newParams.toString();
      redirect(`/publications/${dest}${qs ? '?' + qs : ''}`);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col">
      <main className="grow">
        <UniversalEditorialHero pageKey="publications" fallbackConfig={publicationsHeroFallback as any} />
