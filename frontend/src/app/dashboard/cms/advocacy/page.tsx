...
export default function AdvocacyListPage() {
  ...
  const fetchCampaigns = async () => {
    try {
      const data = await initiativeApi.getAll("ADVOCACY");
      setCampaigns(normalizeArray(data, ["campaigns", "items", "results", "data"]));
    } catch (error) {
      console.error("Error fetching advocacy campaigns:", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">
            Manage your advocacy graphics and hashtags.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/dashboard/cms/advocacy/add"
          ...
          {campaigns.length === 0 ? (
            <div className="col-span-full text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
              <p className="text-gray-400">
                No advocacy items found. Create your first one!
              </p>
            </div>
          ) : (
            campaigns.map((campaign) => (
              <div key={campaign.id} className={`relative group rounded-2xl overflow-hidden transition-all ${selectedIds.has(campaign.id) ? "ring-2 ring-brand-primary shadow-lg" : "hover:shadow-lg"}`}>
                ...
                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                  ...
                  <Link
                    href={`/dashboard/cms/advocacy/edit/${campaign.id}`}
