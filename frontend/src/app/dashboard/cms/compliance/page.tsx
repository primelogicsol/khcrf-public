...
import { FaPlus, FaTrash, FaSave, FaBalanceScale, FaCheckSquare, FaRegSquare, FaUpload } from "react-icons/fa";
...
const DEFAULT_ITEM: ComplianceItem = {
  title: "",
  description: "",
  icon: "FaBalanceScale",
};
...
export default function ComplianceCMS() {
  ...
  const fetchContent = async () => {
    try {
      const data = await cmsApi.getContent("compliance-items");
  ...
  const removeSelected = () => {
    ...
    if (confirm(`Are you sure you want to remove ${selectedIndices.size} item(s)?`)) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await cmsApi.updateContent("compliance-items", {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <FaBalanceScale data-ui-icon  className="" />
            Compliance Items
          </h1>
          <p className="text-gray-500">
            Manage compliance items displayed on the Compliance page
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title="Upload JSON file to bulk add items"
          >
          ...
          >
            ...
            <div className="space-y-4">
              <Input
                ...
                placeholder="e.g. Section 8 Compliance"
              />
