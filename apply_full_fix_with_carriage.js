...
// 1. Add getHearingDate helper and setup variables at the top of component
const setupVars = `
  ...
  const formalHearings = hearings.filter((h: any) => h.eventType === 'PUBLIC_HEARING');
...
content = content.replace(/  const completedHearings = hearings\.filter\(\(h: any\) => h\.status === 'COMPLETED'\)\.length;\r?\n  const scheduledHearingsCount = hearings\.length > 0 \? hearings\.length : mockHearingsLength;\r?\n  const nextHearing = [^\n]+\r?\n  const nextHearingText = [^\n]+\r?\n/, setupVars + '\n');


// 2. Fix getEventStatus
const newGetEventStatus = `
...
`;
content = content.replace(/  const getEventStatus = \([\s\S]*?    return 'Upcoming';\r?\n  };\r?\n/, newGetEventStatus);


// 3. Delete formalHearings definition below since it is already defined above
content = content.replace(/  const formalHearings = hearings\.filter\(h => h\.eventType === 'PUBLIC_HEARING'\);\r?\n/, '');

// 4. Remove renderEventGroup entirely
content = content.replace(/  const renderEventGroup = \([\s\S]*?    \};\r?\n/m, '');

// 5. Setup grouped variables for the new UI just before return statement
const groupedVars = `
...
// 6. Replace the Programme Calendar UI
const newUI = `      {/* 6. Programme Calendar Dashboard */}
      {hearings.length > 0 && (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 bg-gray-50 border border-gray-200 rounded-xl p-6">
              ...
              <div className="col-span-2">
                ...
                <span className="text-sm font-bold text-brand-dark">
                  {nextScheduledHearing ? \`\${getHearingDate(nextScheduledHearing)?.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', timeZone: 'Asia/Kolkata' })} · \${nextScheduledHearing.title} · \${nextScheduledHearing.district}\` : 'None Scheduled'}
                </span>
              </div>
            </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col">
                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                              {getHearingDate(m)?.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', timeZone: 'Asia/Kolkata' })?.toUpperCase() || 'TBD'}
                            </span>
                          </div>
                        </div>
                  ...
                  <div key={h.id} className="mb-12 border-b border-gray-200 pb-12">
                    ...
                    <div className="flex items-start gap-4 md:gap-8">
                      <div className="w-12 md:w-16 flex-shrink-0 text-center pt-1">
                        ...
                        <span className="text-xs font-bold text-gray-900">{getHearingDate(h)?.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', timeZone: 'Asia/Kolkata' })?.toUpperCase() || 'TBD'}</span>
                      </div>
                      <div>
                  ...
                  {workflowEvents.filter(h => h.eventType === 'INTERNAL_PANEL').map((h, i) => (
                    <React.Fragment key={h.id}>
                      <div className="flex-shrink-0 w-full md:w-48 bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-col justify-between">
                        <div className="text-[10px] font-bold text-gray-500 mb-2 uppercase tracking-wider">{getHearingDate(h)?.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', timeZone: 'Asia/Kolkata' }) || 'TBD'}</div>
...
`;
content = content.replace(/      \{\/\* 6\. Programme Calendar \*\/\}\r?\n[\s\S]*?      \{\/\* 7\. District Participation Dashboard \*\/\}/m, newUI + '\n\n      {/* 7. District Participation Dashboard */}');
