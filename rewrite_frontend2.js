...
c = c.replace(
  'import { FELLOWSHIP_PROGRAMME } from \'@/config/fellowshipConfig\';\n',
  ''
);
...
c = c.replace(/<main className="bg-white">/g, '<main className="bg-white">{loading ? <div className="p-20 text-center">Loading...</div> : <>');
c = c.replace(/<\/main>/g, '</>}</main>');
