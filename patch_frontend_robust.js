...
const oldStart = c.indexOf('const handleSubmit = async (e: React.FormEvent) => {');
const oldEnd = c.indexOf('useEffect(() => {', oldStart);
if (oldStart !== -1 && oldEnd !== -1) {
  // we want to slice up to the last '};' before useEffect
  const endSlice = c.lastIndexOf('};', oldEnd) + 2;
  c = c.substring(0, oldStart) + newSubmit + '\n\n  ' + c.substring(oldEnd);
}
...
c = c.replace(/<input\s*type=\"file\"\s*accept=\"\.pdf\"\s*required\s*onChange=\{\(e\) => setCvFile\(e\.target\.files\?\.\[0\] \|\| null\)\}\s*className=\"w-full text-sm\"\s*\/>/g, 
...
// fix the success UI
c = c.replace(
  /\{formStatus !== 'idle' && formStatus !== 'submitting' && \([\s\S]*?<\/div>\n\s*\)\}/,
`{formStatus === 'error' && (
