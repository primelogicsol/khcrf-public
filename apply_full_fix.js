...
content = content.replace(/  const completedHearings = hearings\.filter\(\(h: any\) => h\.status === 'COMPLETED'\)\.length;\n  const scheduledHearingsCount = hearings\.length > 0 \? hearings\.length : mockHearingsLength;\n  const nextHearing = [^\n]+\n  const nextHearingText = [^\n]+\n/, setupVars + '\n');
...
`;
content = content.replace(/  const getEventStatus = \([\s\S]*?    return 'Upcoming';\n  };\n/, newGetEventStatus);
