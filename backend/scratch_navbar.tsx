...
export function GlobalHeader({ mode = "solid" }: GlobalHeaderProps) {
  ...
  return (
    <header className="relative w-full z-[var(--z-header)]">
      {/* Original Topbar - Brown, 85% width, aligned right */}
      <div className={`hidden lg:block ${isOverlay ? "bg-transparent" : "bg-white"}`}>
        <div className="container-fluid mx-auto">
          <div className="flex justify-end">
            <div className="bg-[#050A1E] w-[85%] rounded-bl-[10px] flex justify-between items-center py-2 px-10 text-white text-sm font-medium">
              ...
              <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center space-x-4">
                  <Link
                    ...
                    className="hover:text-[#CDAC36] transition font-bold"
                  >
                    Join HCRF
                  </Link>
                  {user ? (
                    ...
                    <>
                      <Link
                        href="/login"
                        className="hover:text-black transition font-bold"
                      >
                        Login
                      </Link>
                      <Link
        ...
        <div className="container-fluid mx-auto">
          ...
          <ul className="hidden xl:flex items-center space-x-8">
            <li>
              <Link
                href="/"
                className="text-xl hover:text-[#CDAC36] transition-all duration-300 ${isOverlay ? "text-white" : "text-[#050A1E]"} transition-all duration-300"
              >
                <FaHome />
              </Link>
            </li>
            {navLinks.map((link) => (
              <li key={link.name} className={`${(link as any).isMega && !(link as any).megaWidth ? '' : 'relative'} group py-6`}>
                <button className="flex items-center font-bold text-[16px] uppercase tracking-[0.05em] transition-all duration-300 ${isOverlay ? "text-white" : "text-[#050A1E]"} group-hover:text-[#CDAC36] transition-all duration-300">
                  {(link as any).href ? (
                ...
                {(link as any).isMega ? (
                  <div 
                    className={`absolute top-full invisible group-hover:visible opacity-0 group-hover:opacity-100 bg-[#FAF9F6] shadow-2xl border-t-2 border-[#CDAC36] translate-y-4 group-hover:translate-y-0 transition-all duration-400 z-[var(--z-mega-menu)] border-b-[8px] border-b-[#050A1E] ${(link as any).megaWidth ? ((link as any).megaAlign === 'right' ? 'right-0' : 'left-0') : 'left-0 w-full'}`}
                    ...
                    <div className={`${(link as any).megaWidth ? 'py-12 px-14' : 'container-fluid mx-auto px-4 md:px-10 py-10'}`}>
                      <div className={`grid gap-y-6 ${(link as any).megaWidth ? 'gap-x-10' : 'gap-x-8'} ${
                        ...
                        {(link as any).columns.map((col: any) => (
                          <div key={col.title}>
                            <h4 className="text-[#050A1E] font-serif font-bold text-lg mb-4 border-b border-[#050A1E]/20 pb-2 whitespace-nowrap">{col.title}</h4>
                            <ul className="space-y-3">
                              {col.links.map((sub: any) => (
                                <li key={sub.name}>
                                  <Link href={sub.href} className="text-[13px] font-bold text-gray-900 hover:text-[#CDAC36] transition-colors block leading-tight whitespace-nowrap hover:bg-gray-100 py-1.5 px-2 -ml-2 rounded">
                                    {sub.name}
                                  </Link>
                                </li>
                              ))}
                  ...
                  <ul className="absolute left-0 top-full invisible group-hover:visible opacity-0 group-hover:opacity-100 bg-white shadow-[0px_10px_60px_0px_rgba(0,0,0,0.1)] min-w-[280px] border-t-2 border-[#CDAC36] translate-y-4 group-hover:translate-y-0 transition-all duration-400 z-[var(--z-mega-menu)]">
                    {link.submenu.map((sub) => (
                      <li key={sub.name}>
                        <Link
                          href={sub.href}
                          className="flex items-center px-8 py-3 text-[14px] font-bold text-gray-700 hover:bg-[#CDAC36] hover:text-white transition-all duration-200 uppercase tracking-widest border-b border-gray-50 last:border-0"
                        >
                          {sub.name}
                          {(sub as any).beta && (
                            <span className="ml-2 bg-[#CDAC36] text-white text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider group-hover:bg-white group-hover:text-[#CDAC36] transition-colors">
                              Beta
                            </span>
                          )}
                        </Link>
          ...
          <div className="flex items-center space-x-3 md:space-x-6 shrink-0 max-[405px]:w-full max-[405px]:justify-end">
            <Link
              ...
              className="bg-[#CDAC36] hover:bg-[#050A1E] text-white px-4 py-2 md:px-6 md:py-2.5 rounded shadow-md hover:shadow-lg transition-all duration-300 font-bold uppercase tracking-wider text-[10px] md:text-sm flex items-center gap-2"
            >
            ...
            {/* Mobile Toggler */}
            <button
              className="xl:hidden bg-white text-[#050A1E] p-2 md:p-3 rounded-lg shadow-md border border-gray-100 hover:bg-[#CDAC36] hover:text-white transition-all duration-300"
      ...
      <div
        className={`fixed top-0 left-0 w-full bg-[#050A1E] z-[var(--z-sticky)] shadow-[0px_10px_60px_0px_rgba(0,0,0,0.1)] transition-transform duration-500 block ${isHeaderSticky ? "translate-y-0" : "-translate-y-full"}`}
      >
        <div className="container-fluid mx-auto px-4 md:px-10 flex justify-between items-center py-2 h-[64px] xl:h-[100px]">
          ...
          <ul className="hidden xl:flex items-center space-x-6">
            <li>
              <Link
                href="/"
                className="text-white hover:text-[#CDAC36] transition-colors duration-300 text-lg"
              >
                <FaHome />
              </Link>
            </li>
            {navLinks.map((link) => (
              <li key={link.name} className={`${(link as any).isMega && !(link as any).megaWidth ? '' : 'relative'} group py-4`}>
                <button className="font-bold text-[15px] uppercase tracking-wider text-white group-hover:text-[#CDAC36] transition-colors duration-300">
                  {(link as any).href ? (
                    <Link href={(link as any).href} className="flex items-center w-full">
                      {link.name}
                    </Link>
                ...
                {(link as any).isMega ? (
                  <div 
                    className={`absolute top-full invisible group-hover:visible opacity-0 group-hover:opacity-100 bg-[#FAF9F6] shadow-2xl border-t-2 border-[#CDAC36] translate-y-2 group-hover:translate-y-0 transition-all duration-400 z-[var(--z-mega-menu)] border-b-[8px] border-b-[#050A1E] ${(link as any).megaWidth ? ((link as any).megaAlign === 'right' ? 'right-0' : 'left-0') : 'left-0 w-full'}`}
                    style={(link as any).megaWidth ? { minWidth: (link as any).megaWidth, width: 'max-content' } : {}}
                  >
                        ...
                        {(link as any).columns.map((col: any) => (
                          <div key={col.title}>
                            ...
                            <ul className="space-y-3">
                              {col.links.map((sub: any) => (
                                <li key={sub.name}>
                                  <Link href={sub.href} className="text-[13px] font-medium text-gray-700 hover:text-[#CDAC36] transition-colors block leading-tight whitespace-nowrap">
                                    {sub.name}
                                  </Link>
                                </li>
                              ))}
                  ...
                  <ul className="absolute left-0 top-full invisible group-hover:visible opacity-0 group-hover:opacity-100 bg-white shadow-2xl min-w-[240px] border-t-2 border-[#CDAC36] translate-y-2 group-hover:translate-y-0 transition-all duration-400">
                    {link.submenu.map((sub) => (
                      <li key={sub.name}>
                        <Link
                          href={sub.href}
                          className="flex items-center px-6 py-2.5 text-[14px] font-bold text-gray-700 hover:bg-[#CDAC36] hover:text-white transition-all duration-200 uppercase tracking-widest border-b border-gray-50 last:border-0"
                        >
                          {sub.name}
                          {(sub as any).beta && (
                            <span className="ml-2 bg-[#CDAC36] text-white text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider group-hover:bg-white group-hover:text-[#CDAC36] transition-colors">
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
            ...
            <Link
              href="/about/donations"
              className="bg-[#CDAC36] hover:bg-[#050A1E] text-white px-3 py-1.5 md:px-5 md:py-2 rounded shadow-md transition-all duration-300 font-bold uppercase tracking-wider text-[10px] md:text-xs flex items-center gap-2"
            >
              <FaHeart className="text-[10px] md:text-xs" /> DONATE
            </Link>
            {/* Mobile Toggler - Sticky */}
            <button
              className="xl:hidden bg-[#CDAC36] text-white p-2 md:p-3 rounded-lg shadow-md border border-white/10 hover:bg-white hover:text-[#CDAC36] transition-all duration-300"
              onClick={() => setIsMobileMenuOpen(true)}
              aria-label="Toggle mobile menu"
              aria-expanded={isMobileMenuOpen}
              aria-controls="mobile-menu-drawer"
        ...
        >
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            ...
            <button
              ...
              className="bg-[#CDAC36] text-white p-2 rounded-full hover:rotate-90 transition-transform shadow-lg"
            >
              <FaTimes />
            </button>
          </div>
            ...
            {user ? (
              <>
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  <FaUserCircle className="text-2xl text-[#CDAC36]" />
                ...
                <Link
                  href="/profile"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex justify-center items-center py-3 rounded-xl bg-[#CDAC36] text-white font-black uppercase tracking-widest text-xs shadow-lg hover:bg-[#050A1E] hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300"
                >
                  Go to Profile
                </Link>
              </>
                    ...
                    {(link as any).isMega ? (
                      (link as any).columns.map((col: any) => (
                        <li key={col.title} className="mb-4 last:mb-0">
                          <h5 className="text-[11px] font-bold text-[#CDAC36] uppercase tracking-widest mb-2 border-b border-gray-100 pb-1">{col.title}</h5>
                            ...
                            {col.links.map((sub: any) => (
                              <li key={sub.name}>
                                <Link
                                  href={sub.href}
                                  onClick={() => setIsMobileMenuOpen(false)}
                                  className="text-[13px] text-gray-800 font-bold hover:text-[#CDAC36] hover:bg-gray-100 flex items-center leading-tight py-2 px-3 -ml-3 rounded-lg transition-colors"
                                >
                                  {sub.name}
                                </Link>
                              </li>
                      ...
                      link.submenu.map((sub: any) => (
                        <li key={sub.name}>
                          <Link
                            href={sub.href}
                            onClick={() => setIsMobileMenuOpen(false)}
                            className="text-[14px] text-gray-500 font-medium hover:text-[#CDAC36] flex items-center"
                          >
                            {sub.name}
                            {(sub as any).beta && (
                              <span className="ml-2 bg-[#CDAC36] text-white text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider">
                                Beta
                              </span>
                            )}
                          </Link>
